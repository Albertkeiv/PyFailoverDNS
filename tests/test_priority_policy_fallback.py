...
def priority_domain():
    ...
    # Принудительно задаём политику priority и цели с приоритетами
...
def test_priority_policy_fallback(priority_domain):
    ...
    # Явно задаём несколько fallback IP
    cfg["fallback"] = ["127.0.0.1", "127.0.0.2"]
    ...
    returned_ips = [str(rr.rdata) for rr in response.rr]
    assert set(returned_ips) == set(fallback), f"Expected fallback IPs: {fallback}, got: {returned_ips}"